...
function makeStudentFromData(data) {
  ...
  if (!student.lastname)
    errors.push({ field: "lastname", message: "Не указано отчество" });

  if (!student.birthday)
    errors.push({ field: "birthday", message: "Не указана дата рождения" });
  if (!student.studyStart)
    errors.push({ field: "studyStart", message: "Не указано начало обучения" });
  if (!student.faculty)
...
module.exports = createServer(async (req, res) => {
  ...
  .on("listening", () => {
    if (process.env.NODE_ENV !== "test") {
      ...
      console.log(
        `POST ${URI_PREFIX} - создать студента, в теле запроса нужно передать объект { name: string, surname: string, lastname: string, birthday: string, studyStart: string, faculty: string}`
      );
      console.log(`GET ${URI_PREFIX}/{id} - получить студента по его ID`);
      console.log(
        `PATCH ${URI_PREFIX}/{id} - изменить студента с ID, в теле запроса нужно передать объект { name?: string, surname?: string, lastname?: string, birthday?: string, studyStart?: string, faculty?: string}`
      );
